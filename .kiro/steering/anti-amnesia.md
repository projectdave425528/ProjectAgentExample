...
| 任務類型 | 路徑 | 判斷標準 |
|---------|------|---------|
| Project 內容 | `main-agent/ProjectRecord/{active-project}/progress-marker.md` | 涉及寫 code、改 ProjectRecord、派 Assignment |
| Project 外內容 | `{當前 Agent 目錄}/progress-marker.md` | 改 steering、hooks、通用操作 |

判斷方式：
- 操作對象喺 `ProjectRecord/{project}/` 入面 → Project 路徑
- 操作對象喺 `.kiro/`、`Agents/`、`UserConfig/` 等 → Agent 目錄路徑

Agent 目錄 = 你當前運行嘅 workspace 嘅根目錄（即放 `.kiro/steering/` 嗰個層級）。
...
任何 Agent 都有可能因為長 session 觸發 context compaction。
Compaction 後你唔會知道自己失憶咗，會重頭開始做已完成嘅工作。
每個 Step 讀一次 marker（~20 tokens）確保你永遠知道做到邊。
