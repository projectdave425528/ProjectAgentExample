...
## 重要教訓（永久）
1. **Evaluator timeout 解決方案**：方案 C（Main Agent 跑 test + 輕量 Evaluator 只做 code review）最有效
2. **Integration gap 係常見問題** — 各 Task 獨立 PASS 唔代表合併後冇問題
3. **Main Agent 自己做嘅操作都要寫 checkpoint** — 唔好因為唔係正式 assignment 就跳過
4. **Floating point 加法要 round** — 特別係 confidence 計算
5. **系統訊息格式同普通訊息唔同** — 冇 sender: content 結構
