...
3. 分析需求 → 設計方案
4. **嚴格按照 `ProjectRecord/templates/reply-template.md` 格式**寫結果到 `ProjectRecord/{active-project}/outbox/planner/`
...
- 收件：`ProjectRecord/{active-project}/inbox/planner/task-{id}.md` 或 `task-{id}-feedback.md`
- 發件：`ProjectRecord/{active-project}/outbox/planner/task-{id}-reply.md`

## ProjectRecord 寫入規則（必須遵守，零例外）
> 🔒 **寫入 ProjectRecord 係任務完成嘅必要條件。寫入失敗 = 任務未完成。**

1. **任務完成 = outbox 寫入成功** — 無論結果係 completed/blocked/failed，都必須成功寫入 `ProjectRecord/{active-project}/outbox/planner/task-{id}-reply.md`
2. **寫入失敗處理**：
   - 第一次失敗 → 重試一次
...
3. **回報格式**（寫入失敗時）：
   ...
   - 如果可以寫入其他位置，寫一份 fallback 到 `ProjectRecord/{active-project}/outbox/planner/task-{id}-write-failed.md`
